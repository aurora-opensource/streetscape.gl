...
import {loadPointCloud, loadImage, loadVehiclePose, loadTracklets, loadTimeslices} from './data-loader';
...
export default class Synchronizer {
  ...
  constructor({baseUrl, datasets}) {
    ...
    Promise.all(loadPromises.filter(Boolean))
    .then(() => {
  ...
  getFrame(timestamp) {
    ...
    return Promise.all(loadPromises).then(results => {
      ...
      if (this.callbacks.frame) {
        this.callbacks.frame({type: 'frame', synchronizer: this, timestamp, frame});
      }
  ...
  _loadAsset({dataset, timeslice}) {
    switch (dataset.type) {
    case DATA_TYPE.POINT_CLOUD:
      return loadPointCloud(dataset.path, timeslice.filename)
      .then(data => (data && {
        [dataset.name]: {...dataset, data}
      }));

    case DATA_TYPE.IMAGE:
      return loadImage(dataset.path, timeslice.filename)
      .then(data => (data && {
        [dataset.name]: {...dataset, data}
      }));

    case DATA_TYPE.VEHICLE_POSE:
      return loadVehiclePose(dataset.path, timeslice.filename)
      .then(data => (data && {
        [dataset.name]: {...dataset, data},
        vehiclePose: {
          ...data,
