...
const {experimental: {Pose}} = require('math.gl');
...
const parseArs = require('./parsers/parse-args');
const generateMetadata = require('./parsers/generate-metadata');
const {DEFAULT_ROOT, DEFAULT_DATE, DEFAULT_DRIVE, STREAMS, STREAMS_METADATA} = require('./parsers/constants');
...
function getArgs() {
  const args = parseArs();
  const basePath = path.join(runPath, (args.root || DEFAULT_ROOT), args.date);
  const disableStreams = (args.disable_streams || '')
    .split(',')
    .reduce((resMap, name) => {
      resMap[name] = name;
      return resMap;
  ...
  return {
    ...
    outdir: args.outdir || path.join(basePath, 'generated'),
    disableStreams
  }
}
...
function generateTrajectory({frameDirs, trajectoryFileName, getMotion, getTrajectory}) {
  ...
  fs.writeFileSync(path.join(frameDir, trajectoryFileName), JSON.stringify(trajectory), {flag: 'w'});
    ...
    getTrajectory: (motions, initialFrame) => {
      ...
      if (motions && motions.length > 1) {
        for (let i = 0; i < motions.length; i++) {
          ...
          const transformMatrix = new Pose(currVehiclePose).getTransformationMatrixFromPose(new Pose(initialVehiclePose));

          // filter the tracklets (in frame + i_step_forward) but not in current frame
          tracklets.filter(t => trackletsMap[t.id])
            .forEach(t => {
...
function assembleFrame(frameDirs, disableStreams) {
  ...
  frameDirs.forEach((frameDir, i) => {
    ...
    fs.writeFileSync(frameFilePath, JSON.stringify(frameMap, null, 2), {flag: 'w'});
    const end = Date.now();
