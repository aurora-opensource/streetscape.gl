...
const {experimental} = require('math.gl');
...
function generateJsonFiles(getPath, tracklets) {
  tracklets.forEach((pose, i) => {
    ...
    fs.writeFileSync(poseFilePath, JSON.stringify(pose, null, 2), {flag: 'w'});
  });
}

function parse(originDataPath, getPath) {
  console.log('processing tracklets');
  ...
  console.log('processing tracklets done');
}
