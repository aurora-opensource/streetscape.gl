...
module.exports = async function main(args) {
  const {inputDir, samplesDir, disabledStreams, fakeStreams, frameLimit} = args;

  const sceneName = `scene-${zeroPaddedPrefix(args.scene, 4)}`;
  ...
  const converter = new NuTonomyConverter(inputDir, outputDir, samplesDir, {
    disabledStreams,
    sceneName,
    fakeStreams
  });

  console.log(`Converting NuScenes data at ${inputDir}`); // eslint-disable-line
  ...
  for (let i = 0; i < limit; i++) {
    const xvizFrame = await converter.convertFrame(frames[i]);
    if (xvizFrame) {
