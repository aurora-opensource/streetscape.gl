...
const {delta_time_ms, extract_zip_from_file} = require('./utils/serve');
...
function getFrameMetadata(index, data_directory) {
  const frameName = getFrameName(index)
    .find(filename => {
...
class ConnectionContext {
  ...
  sendNextFrame() {
    this.frame_tracking.sendInterval = setTimeout(() => this.sendFrame(), this.settings.send_interval);
  }
  ...
  logMsgSent(send_time, index, real_index, tag) {
    ...
    console.log(` < Frame(${tag}) ${index}:${real_index} in self: ${t_msg_send_time_ms}ms start: ${t_from_start_ms}ms`);
  }
}
